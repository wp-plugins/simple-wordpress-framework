...
add_action('admin_menu', 'swpframework_menu');

function swpframework_menu() {
...
/*  
	...
	<file>			the name/identifier that is added to the end of the menu's URL.  For example: /admin.php?page=<file>.
					...
					swpframework-home, or swpframework-options, etc.
	-----------------------------------------------------------------------------------------------------------------------
	...
	<parent>		filename of the core WordPress admin file that supplies the top-level menu in which you want to insert 
					your submenu, or your plugin file if this submenu is going into a custom top-level menu.
		  	 		...
		  	 		I used the constant defined in swpframework.php, "MYPLUGIN_NAME" to assign the the <parent> to every menu
...
*/
 	...
 	$icon_url = plugins_url('images/generic-tango.png', __FILE__);
	 
 	## add_menu_page (page_title, menu_title, user_level, file, [function], [icon_url]);  //  adds a new top-level menu:
	add_menu_page('', 'SWP Framework', 'administrator', MYPLUGIN_NAME, 'swpframework_submenu_welcome', $icon_url) ;
	
	
  	## add_submenu_page (parent, page_title, menu_title, user_level, file, [function]);   // adds a new submenu to the custom top-level menu    
	add_submenu_page(MYPLUGIN_NAME, 'Welcome to SWP Framework', 'Welcome', 'administrator', MYPLUGIN_NAME, 'swpframework_submenu_welcome');      
	add_submenu_page(MYPLUGIN_NAME, 'Page Title: Submenu #2', 'Submenu #2', 'administrator', 'swpframework-submenu-2', 'swpframework_submenu_2');
	add_submenu_page(MYPLUGIN_NAME, 'Page Title: Submenu #3', 'Submenu #3', 'administrator', 'swpframework-submenu-3', 'swpframework_submenu_3');
	add_submenu_page(MYPLUGIN_NAME, 'Page Title: Submenu #4', 'Submenu #4', 'administrator', 'swpframework-submenu-4', 'swpframework_submenu_4');
	add_submenu_page(MYPLUGIN_NAME, 'Page Title: Submenu #5', 'Submenu #5', 'administrator', 'swpframework-submenu-5', 'swpframework_submenu_5');
	add_submenu_page(MYPLUGIN_NAME, 'SWP Framework Options', 'Options', 'administrator', 'swpframework-options', 'swpframework_submenu_options');
	
}


function swpframework_submenu_welcome() {
	
  echo '<div class="wrap">';
  echo '<p>Welcome to my Sublevel Page 1.  Here you can put an introduction about your plugin.  Just replace this text with new HTML code.</p>';
  echo '</div>';
 
}

function swpframework_submenu_2() {
	
  echo '<div class="wrap">';
  echo '<p>Welcome to my Sublevel Page 2.  Here you can put an introduction about your plugin.  Just replace this text with new HTML code.</p>';
  echo '</div>';

}

function swpframework_submenu_3() {
	
  echo '<div class="wrap">';
  echo '<p>Welcome to my Sublevel Page 3.  Here you can put an introduction about your plugin.  Just replace this text with new HTML code.</p>';
  echo '</div>';

}

function swpframework_submenu_4() {
	
  echo '<div class="wrap">';
  echo '<p>Welcome to my Sublevel Page 4.  Here you can put an introduction about your plugin.  Just replace this text with new HTML code.</p>';
  echo '</div>';

}

function swpframework_submenu_5() {
	
  echo '<div class="wrap">';
  echo '<p>Welcome to my Sublevel Page 5.  Here you can put an introduction about your plugin.  Just replace this text with new HTML code.</p>';
  echo '</div>';

}

function swpframework_submenu_options() {
  ## A function that already exists in the options.php file is being called. You will find different
  ##  samples of HTML code, to assist you in designing your options page.
  	
  swpframework_options_page();}
