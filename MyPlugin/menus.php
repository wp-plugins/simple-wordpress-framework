...
function myplugin_menu() {
	...
	myplugin-home, or myplugin-options, etc.
	-----------------------------------------------------------------------------------------------------------------------
	...
	I used the constant defined in myplugin.php, "MYPLUGIN_NAME" to assign the the <parent> to every menu
	...
	*/
	...
	add_menu_page('', 'My Plugin', 'administrator', MYPLUGIN_NAME, 'myplugin_submenu_welcome', $icon_url) ;

	## add_submenu_page (parent, page_title, menu_title, user_level, file, [function]);   // adds a new submenu to the custom top-level menu
	add_submenu_page(MYPLUGIN_NAME, 'Welcome to SWP Framework', 'Welcome', 'administrator', MYPLUGIN_NAME, 'myplugin_submenu_welcome');
	add_submenu_page(MYPLUGIN_NAME, 'Page Title: Submenu #1', 'Submenu #1', 'administrator', 'myplugin-submenu-1', 'myplugin_submenu_1');
	add_submenu_page(MYPLUGIN_NAME, 'Page Title: Submenu #2', 'Submenu #2', 'administrator', 'myplugin-submenu-2', 'myplugin_submenu_2');
	add_submenu_page(MYPLUGIN_NAME, 'SWP Framework Options',  'Options',    'administrator', 'myplugin-submenu-options', 'myplugin_submenu_options');

}


function myplugin_submenu_welcome() {

	echo '<div class="wrap">';
	echo myplugin_header_icon().'<h2>Welcome to My Plugin</h2>';

	echo '<p>Welcome to My Plugin.  You can place HTML code, include/require a PHP file, or call a function function from this area.</p>';

	echo ' <p>You are viewing the Welcome page for  <a href="http://wordpress.org/extend/plugins/myplugin-template/">Simple
...
function myplugin_submenu_1() {

	echo '<div class="wrap">';
	echo  myplugin_header_icon() . '<h2>Sublevel Page #1</h2>';
...
function myplugin_submenu_2() {

	echo '<div class="wrap">';
	echo  myplugin_header_icon() . '<h2>Sublevel Page #2</h2>';
...
function myplugin_submenu_options() {

	myplugin_options_page_submenu();  ## Calls a function located in the options.php file. You will find different
